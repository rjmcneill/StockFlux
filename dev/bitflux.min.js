...
Math.max(0,g.left(f,d)-1),Math.min(g.right(f,e)+1,f.length));return h}function g(a,c,d,e){arguments.length<4&&(e=1);var f=b.util.extent().fields("date")(d),g=b.util.extent().fields(b.util.fn.identity)(c),h=a.distance(f[0],f[1]),i=e*a.distance(g[0],g[1]),j=h>i?[a.offset(f[1],-i),f[1]]:f;return j}function h(b,c){var d=a.max(b,function(a){return a.getTime()}),e=a.max(c,function(a){return a.date.getTime()});return d===e}function i(a,c){a.secondaries.style("flex",c.secondaries().charts().length),a.overlaySecondaries.style("flex",c.secondaries().charts().length);var d=parseInt(a.app.select(".head-row").style("height"),10);sa||(d+=parseInt(a.app.select(".head-row").style("padding-top"),10)+parseInt(a.app.select(".head-row").style("padding-bottom"),10)+parseInt(a.app.select(".head-row").style("margin-bottom"),10),sa=!0);var e=b.util.innerDimensions(a.app.node()).height-d;a.chartsAndOverlay.style("height",e+"px"),c.xAxis().dimensionChanged(a.xAxis),c.nav().dimensionChanged(a.navbar),c.primary().dimensionChanged(a.primary),c.secondaries().charts().forEach(function(b){b.option.dimensionChanged(a.secondaries)})}function j(){return++ta}function k(a){return c(a).width()}
...
function m(a,b,c){var d,e,f=null,g=0;c||(c={});var h=function(){g=c.leading===!1?0:(new Date).getTime(),f=null,e=a.apply(this,d),f||(d=null)};return function(){var i=(new Date).getTime();g||c.leading!==!1||(g=i);var j=b-(i-g);return d=arguments,0>=j||j>b?(f&&(clearTimeout(f),f=null),g=i,e=a.apply(this,d),f||(d=null)):f||c.trailing===!1||(f=setTimeout(h.bind(this),j)),e}}function n(c){function d(a){
// If zooming, and about to pan off screen, do nothing
return a[0]>0&&a[1]<0}function e(){k.translate([0,0]),k.scale(1)}function f(a,b,c){return Math.min(Math.max(a,b),c)}function g(b,c,d){var e=b;return i(c[0].date)>0&&(e[1]=i.invert(i(b[1])+i(c[0].date))),e[0]=a.max([d[0],e[0]]),e[1]=a.min([d[1],e[1]]),e}function h(h){var o=b.util.extent().fields("date")(h.datum().data),p=i(o[0]),q=i(o[1]),r=[p,q-c];k.x(i).on("zoom",function(){var b=a.event.translate,c=b[0],p=d(r);c=f(c,-r[1],-r[0]),k.translate([c,0]);var q=1===k.scale(),s=1!==k.scale();if(q&&l||s&&m){var t=i.domain();p?t=o:s&&n&&(t=ua.domain.moveToLatest(h.datum().discontinuityProvider,t,h.datum().data)),t=g(t,h.datum().data,o),t[0].getTime()!==t[1].getTime()?j.zoom(t):
// Ensure the user can't zoom-in infinitely, causing the chart to fail to render
// #168, #411
e()}else e()}),h.call(k).on("dblclick.zoom",null)}var i,j=a.dispatch("zoom"),k=a.behavior.zoom(),l=!0,m=!0,n=!0;return h.allowPan=function(a){return arguments.length?(l=a,h):l},h.allowZoom=function(a){return arguments.length?(m=a,h):m},h.trackingLatest=function(a){return arguments.length?(n=a,h):n},h.scale=function(a){return arguments.length?(i=a,h):i},a.rebind(h,j,"on"),h}function o(){function c(a,b){a.select(".plot-area").selectAll(".e, .w").classed("hidden",b)}function d(a){return a.extent()[0]-a.extent()[1]===0}function e(a,c){var d=a.selectAll("defs").data([0]).enter().append("defs");d.html('<linearGradient id="brush-gradient" x1="0" x2="0" y1="0" y2="1">               <stop offset="0%" class="brush-gradient-top" />               <stop offset="100%" class="brush-gradient-bottom" />           </linearGradient>           <mask id="brush-mask">               <rect class="mask-background"></rect>           </mask>'),a.select(".mask-background").attr({width:g,height:j}),t.domain(b.util.extent().fields("date")(c)),u.domain(b.util.extent().fields(["low","high"]).pad(q)(c)),a.select("mask").datum(c).call(E)}function f(a){var f=a.datum();D.bucketSize(Math.max(f.data.length/r,1));var h=D(f.data);t.discontinuityProvider(f.discontinuityProvider),w.discontinuityProvider(f.discontinuityProvider),e(a,h),w.domain(f.viewDomain);var i=ua.domain.filterDataInDateRange(b.util.extent().fields("date")(h),h),j=b.util.extent().fields(["low","high"]).pad(q)(i);v.xDomain(b.util.extent().fields("date")(h)).yDomain(j),z.on("brush",function(){var b=d(z);
...
var k=n(g).scale(w).trackingLatest(f.trackingLatest).allowPan(!1).on("zoom",function(a){s[qa.viewChange](a)});a.select(".plot-area").datum({data:f.data,discontinuityProvider:f.discontinuityProvider}).call(k)}var g,h=100,i=40,j=h-i,k=1,l=j-k,m=l,o=k+m/2,p=2,q=[0,.04],r=200,s=a.dispatch(qa.viewChange),t=b.scale.dateTime(),u=a.scale.linear(),v=b.chart.cartesian(t,u).yTicks(0).margin({bottom:i}).xOuterTickSize(0).yOuterTickSize(0),w=b.scale.dateTime(),x=b.series.area().xValue(function(a){return a.date}).y1Value(function(a){return a.close}).y0Value(function(){return u.domain()[0]}),y=b.series.line().xValue(function(a){return a.date}).yValue(function(a){return a.close}),z=a.svg.brush(),A=b.series.multi().series([x,y,z]).decorate(function(a){var b=a.enter();a.selectAll(".background, .extent").attr("height",l).attr("y",k),
